...
public class CalculatorServiceParameterizedTest {
    private final CalculatorServic calcUnderTest = new CalculatorServic();
    ...
    public void testSumResult(int num1, int num2, int expectedResult) { // в аргументах также результат
        long result = calcUnderTest.plusCalculator(num1, num2); //вызов метода
    ...
    public void testSubtractResult(int num1, int num2, int expectedResult) {
        long result = calcUnderTest.minusCalculator(num1, num2);
        assertEquals(expectedResult, result);
    }
    @ParameterizedTest
    @CsvSource({"2,2,4", "5,8,40"})
    public void testMultiplyResult(int num1, int num2, int expectedResult) {
        long result = calcUnderTest.multiplyCalculator (num1, num2);
        assertEquals(expectedResult, result);
    }
    ...
    public void testDivideResult(int num1, int num2, double expectedResult) {
        Number result = calcUnderTest.divideCalculator(num1, num2);
        assertEquals(expectedResult, result);
    }
    @ParameterizedTest
